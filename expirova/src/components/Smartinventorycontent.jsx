...
const [inventoryItems, setInventoryItems] = useState(() => {
  ...
  const categories = ["All", "Medicine", "Food & Dairy", "Medical Supply", "IV Fluid", "Vaccine"];
  const locations = ["All", "Unit 4, Fridge B", "Cold Chain Lab 1", "Pharmacy Shelf G-2", "ER Supply Bay", "Rack A-3", "Storage Room"];

  const statusPills = [
    { label: "All", active: "bg-slate-800 text-white", inactive: "bg-white text-slate-600 border border-slate-200" },
    { label: "URGENT", active: "bg-red-600 text-white", inactive: "bg-white text-red-600 border border-red-200" },
    { label: "WARNING", active: "bg-amber-500 text-white", inactive: "bg-white text-amber-600 border border-amber-200" },
    { label: "STABLE", active: "bg-[#0d5f94] text-white", inactive: "bg-white text-[#0d5f94] border border-blue-200" },
  ];
   ...
   <main className="w-full bg-[#f8fafc] px-4 pt-24 pb-28">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl p-4 shadow-sm">
          ...
          <h2 className="text-3xl font-bold text-slate-900">{inventoryItems.length}</h2>
        </div>
    ...
    expand_more
        ...
        <div className="flex items-center justify-between mt-3 px-1">
          <p className="text-sm text-gray-400">
            Showing <span className="font-semibold text-slate-700">{filteredItems.length}</span> of {inventoryItems.length} items
          </p>
          {(categoryFilter !== "All" || locationFilter !== "All" || statusFilter !== "All" || search) && (
            <button
              onClick={() => {
         ...
         {filteredItems.map((item, index) => (
            ...
            >
              <div className="flex justify-between mb-3">
                <span className={`${item.statusColor} px-2.5 py-1 rounded-full text-xs font-semibold tracking-wide`}>
                  {item.status}
                </span>
                <div className="relative">
  ...
  Remove
              ...
              <div className="mt-6 pt-4 border-t flex justify-between items-end">
                <div>
                  ...
                  <p className={`text-xl font-bold ${item.valueColor}`}>{item.expiry}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-gray-400 mb-0.5">Location</p>
                  <p className="font-medium text-slate-700 text-sm">{item.location}</p>
                </div>
              </div>
            </div>
          ))}
